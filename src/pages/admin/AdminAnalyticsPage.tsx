      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-[#E5E5E7] pb-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-display font-bold text-[#1D1D1F] flex items-center gap-2">
            <BarChart3 className="w-6 h-6 text-[#0071E3]" /> Network Telemetry & System Analytics
          </h1>
        ...
        <span className="px-3 py-1 rounded-full text-xs font-mono font-bold bg-[#0071E3]/10 text-[#0071E3] border border-[#0071E3]/20">
        ...
        <GlassCard className="p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-[#E5E5E7] pb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-display font-bold text-base text-[#1D1D1F]">System Actions Real Breakdown</h3>
            </div>
            <span className="text-[10px] font-mono text-[#6E6E73]">Audit Trail ({auditLogs.length})</span>
          </div>

          {actionList.length === 0 ? (
            <div className="p-8 text-center text-xs text-[#6E6E73] italic bg-[#F5F5F7] rounded-2xl border border-[#E5E5E7]">
              No audit log activity recorded yet.
            </div>
          ) : (
            <div className="space-y-3">
              {actionList.map((item) => (
                <div key={item.action} className="space-y-1.5 p-3 bg-[#F5F5F7] rounded-xl border border-[#E5E5E7]">
                  <div className="flex justify-between text-xs font-mono">
                    <span className="text-[#1D1D1F] font-bold">{item.action}</span>
                    <span className="text-[#0071E3] font-bold">{item.count} events ({item.percentage}%)</span>
                  </div>
        ...
        <GlassCard className="p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-[#E5E5E7] pb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-display font-bold text-base text-[#1D1D1F]">Whitelisted Hospital Nodes ({totalHospitals})</h3>
            </div>
            <span className="text-[10px] font-mono text-emerald-700">All Nodes Active</span>
          </div>
          ...
          {hospitals.length === 0 ? (
            ...
            <div className="space-y-2.5">
              {hospitals.slice(0, 5).map((h: any) => (
                <div key={h.id} className="p-3 bg-[#F5F5F7] border border-[#E5E5E7] rounded-xl flex items-center justify-between text-xs">
                  <div>
                    <span className="font-bold text-[#1D1D1F] block">{h.name}</span>
                    <span className="text-[10px] text-[#6E6E73] font-mono">{h.address || 'Network Node'}</span>
                  </div>
      ...
      <GlassCard className="p-6 space-y-4">
        <div className="flex items-center justify-between border-b border-[#E5E5E7] pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-display font-bold text-base text-[#1D1D1F]">Physician Verification Real Status</h3>
          </div>
          <span className="text-[10px] font-mono text-[#6E6E73]">Licensing Audit</span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-4 bg-[#F5F5F7] rounded-2xl border border-[#E5E5E7] space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-emerald-800 font-mono">Verified Practitioners</span>
              <CheckCircle2 className="w-4 h-4 text-emerald-600" />
            </div>
            <div className="text-3xl font-black text-[#1D1D1F] font-mono">{verifiedDoctors}</div>
            <p className="text-[11px] text-[#6E6E73]">Authorized to inspect records and issue digital prescriptions</p>
          </div>

          <div className="p-4 bg-[#F5F5F7] rounded-2xl border border-[#E5E5E7] space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-amber-800 font-mono">Pending Verifications</span>
              <ShieldAlert className="w-4 h-4 text-amber-600" />
            </div>
            <div className="text-3xl font-black text-[#1D1D1F] font-mono">{pendingDoctors}</div>
            <p className="text-[11px] text-[#6E6E73]">Awaiting administrator licensing and hospital verification</p>
          </div>
        </div>
      </GlassCard>
    </div>
